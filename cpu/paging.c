...
page_t *get_page_ptr_in_page_table(uint32_t address, int make, page_directory_t *dir)
{
    ...
    {
//        printf("found page - %d, %d, 0x%08x \n", table_idx, address %1024, dir->tables[table_idx]->pages[address%1024]);
...
//    printf("(uint32_t)dir->tablesPhysical 0x%08x,phys 0x%08x, phys + offset 0x%08x, pys addr  0x%08x\n", (uint32_t)dir->tablesPhysical, phys, phys + offset, &phys);
